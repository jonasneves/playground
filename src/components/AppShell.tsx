...
export function AppShell() {
  ...
  const loadApps = async () => {
    try {
      ...
      const appData: AppData[] = appDirs.map(app => ({
        name: app.name,
        path: app.path,
        manifest: null
      }));
  ...
  const loadApp = useCallback(async (appPath: string, appName: string) => {
    try {
      ...
      track('app_launch', { appName, appPath });
  ...
  if (loading && apps.length === 0) {
    return (
      <div>
        <UserMenu />
        <div style={{
          background: `linear-gradient(135deg, ${AppConfig.branding.theme.primary} 0%, ${AppConfig.branding.theme.secondary} 100%)`,
          color: 'white',
          padding: '48px 20px',
          textAlign: 'center',
          marginBottom: '48px'
          ...
          <h1 style={{
            ...
            <Rocket size={32} /> {AppConfig.branding.title}
          </h1>
